...
class Hierarchical extends JFrame implements GLEventListener, KeyListener, MouseListener, MouseMotionListener, ActionListener {
    ...
    private final float mat_diffuse[] = {0f, .1f, .2f, 1f};
    private final float orange[] = {.5f, .3f, .3f, 1f};
    private final float green[] = {.2f, .8f, .3f, 1f};
    private final float purple[] = {.3f, .2f, .9f, 1f};
    private final float black[] = {.0f, .0f, .0f, 1f};
    private final float blue[] = {.0f, .3f, .5f, 1f};
    private final float mat_diffuse2[] = {.7f, .2f, .2f, 1f};    
    private final float mat_specular[] = {.3f, .2f, .2f, 1f};
 ...
 /* Define more models you need for constructing your scene */
    ...
    private objModel globe = new objModel("globe.obj");
    ...
    private float example_rotateT1 = 0.f;
 ...
 /* Below is an example of a rotating bunny
        ...
        gl.glPushMatrix();	// push the current matrix to stack
	        gl.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_DIFFUSE, orange, 0);
	        ...
	        gl.glPushMatrix();
	        	...
	        	gl.glRotatef(example_rotateT, 0, 0, 1);
	        ...
	        gl.glPushMatrix();
        		gl.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR, purple, 0);
	        	gl.glScalef(.15f, .15f, .15f);
		        gl.glTranslatef(.3f, -4.2f, .2f);
		        shoe.Draw();
	        gl.glPopMatrix();
	        ...
	        gl.glPushMatrix();
				gl.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_DIFFUSE, orange, 0);
	        	gl.glScalef(.2f, .2f, .2f);
	        	gl.glTranslatef(-2.0f, -3.0f, 3.0f);
		        gl.glRotatef(example_rotateT*20, 0, 1, 0);
		        armadillo.Draw();
		        gl.glPushMatrix();
        			gl.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_DIFFUSE, purple, 0);
			        gl.glTranslatef(0, .8f, 0);
			        gl.glRotatef(example_rotateT*5, 0, 1, 0);
		        	globe.Draw();
		        	gl.glPushMatrix();
				        gl.glTranslatef(0, 1.2f, .2f);
				        gl.glRotatef(example_rotateT, 0, 1, 0);
			        	bottle.Draw();
			        gl.glPopMatrix();
		        gl.glPopMatrix();
        ...
        gl.glPushMatrix();
	        gl.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR, orange, 0);
...
//      gl.glPushMatrix();
//      gl.glRotatef(example_rotateT1, 1, 0, 0);
//      glut.glutSolidCube(1.0f);

        /* increment example_rotateT */
        if (animator.isAnimating()) {
            example_rotateT += 1.0f * animation_speed;
            example_rotateT1 -= 1.0f * animation_speed;
        }
    }
